...
function setEncryptMode(on) {
    ...
    btn.textContent = on ? 'upload' : 'upload unencrypted';
    btn.classList.toggle('plain', !on);
}
...
async function doUpload() {
    ...
    try {
        ...
        output = new Uint8Array(magic.length + salt.length + nonce.length + ciphertext.length);
        output.set(magic, 0);
        ...
        output = new Uint8Array(fileBuffer);
        }
